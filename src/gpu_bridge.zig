...
pub const GpuBridge = struct {
    ...
    pub fn register(self: *GpuBridge, ctx: *Context) !void {
        ...
        // gpuRequestAdapter() — creates GraphicsContext on first call
        ...
        // gpuRequestDevice(adapterId) → device handle (allocated on demand)
        ...
        // gpuGetQueue(deviceId) → queue handle (allocated on demand)
...
test "gpuRequestAdapter returns adapter handle ID as number" {
    ...
    // Don't call bridge.deinit() — destroyTestGpuContext handles cleanup
