...
pub fn linkSystemDeps(b: *std.Build, compile_step: *std.Build.Step.Compile) void {
    switch (compile_step.rootModuleTarget().os.tag) {
        ...
        .linux => {
            if (compile_step.rootModuleTarget().abi == .android) {
                compile_step.linkSystemLibrary("android");
                compile_step.linkSystemLibrary("log");
            }
        },
        else => {},
    }
        ...
        .linux => {
            if (target.abi == .android) {
                if (target.cpu.arch.isAARCH64()) {
                    if (b.lazyDependency("dawn_aarch64_linux_android", .{})) |dawn_prebuilt| {
                        compile_step.addLibraryPath(dawn_prebuilt.path(""));
                    }
                } else if (target.cpu.arch.isX86()) {
                    if (b.lazyDependency("dawn_x86_64_linux_android", .{})) |dawn_prebuilt| {
                        compile_step.addLibraryPath(dawn_prebuilt.path(""));
                    }
                }
            } else {
...
pub fn checkTargetSupported(target: std.Target) bool {
    const supported = switch (target.os.tag) {
        ...
        .linux => (target.cpu.arch.isX86() or target.cpu.arch.isAARCH64()) and (target.abi.isGnu() or target.abi == .android),
        .macos => blk: {
